...
**LMArena —— ToS 禁止抓取。** 没有公开 API，robots.txt 屏蔽 `/api/`，
ToS 专门禁止用自动化手段抓取「包括 AI Service 名称、标识、版本在内的数据」。
官方给出的合规路径是用 HuggingFace 上的对战数据自行复算 Elo，但那个数据集停更在 2025-08。
替代方案是 Epoch 收录的 WebDev Arena Elo，同为 Elo 制、覆盖到 2026-08。
...
每小时一次，带 `If-None-Match` 条件请求（命中 304 时零流量）。

依据：models.dev 的上游 CI 就是每小时一跑，OpenRouter 的 CDN `max-age=300`，
抓得比一小时更密集没有意义。实测 models.dev 收录新模型的时延是 0–24 小时，
